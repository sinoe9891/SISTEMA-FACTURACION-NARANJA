...
try {
    ...
    if (!empty($_POST['_solo_estado'])) {
        ...
        if (!empty($_FILES['archivo_adjunto']['name'])) {
            ...
            $prevStmt = $pdo->prepare("SELECT archivo_adjunto FROM gastos WHERE id=? AND cliente_id=?");
            $prevStmt->execute([$gasto_id, $cid]);
            $prev = $prevStmt->fetchColumn();
            if ($prev && file_exists($uploadDir.$prev)) @unlink($uploadDir.$prev);
            $arch_adj = 'gasto_'.$cid.'_'.time().'_'.bin2hex(random_bytes(4)).'.'.$ext;
        ...
        $sets   = ['estado=?'];
        $params = [$estado];
        if ($fecha_real)  { $sets[] = 'fecha=?';           $params[] = $fecha_real; }
        if ($met_pago)    { $sets[] = 'metodo_pago=?';     $params[] = $met_pago; }
        if ($notas_pago)  { $sets[] = 'notas=?';           $params[] = $notas_pago; }
    ...
    if ($usarGrupo) {
        ...
        ")->execute([
            ...
            $frecuencia, $dia_pago, $dia_pago_2, $fecha_venc,
            $tipo, $metodo_pago, $proveedor, $factura_ref, $notas, $estado,
            $grupoId, $cid
        ]);
        $pdo->commit();
