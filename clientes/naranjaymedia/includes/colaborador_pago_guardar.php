...
// Compatibilidad: si USUARIO_ID no está definido como constante, obtenerlo de sesión
if (!defined('USUARIO_ID')) {
    define('USUARIO_ID', $_SESSION['usuario_id'] ?? 0);
}
...
try {
    ...
    $cid = defined('CLIENTE_ID') && USUARIO_ROL !== 'superadmin'
        ? (int)CLIENTE_ID
    ...
    if ($tipo === 'quincenal' && !in_array($quincena, [1, 2], true))
        throw new Exception("Para pago quincenal debes indicar quincena 1 ó 2.");
    ...
    if (!empty($cuotas_ids_post)) {
        ...
        $st  = $pdo->prepare("
            ...
            WHERE c.id IN ($ph2)
              AND p.colaborador_id = ? AND p.cliente_id = ?
    ...
    if (!empty($bonos_ids_post)) {
        ...
        $stB = $pdo->prepare("SELECT id, monto_total, descripcion FROM colaborador_prestamos
            WHERE id IN ($phB) AND colaborador_id = ? AND cliente_id = ?
    ...
    if (!empty($viaticos_ids_post)) {
        ...
        $stV = $pdo->prepare("SELECT id, monto_total, descripcion FROM colaborador_prestamos
            WHERE id IN ($phV) AND colaborador_id = ? AND cliente_id = ?
    ...
    // Categoría automática (busca "nomina" o usa la del colaborador)
    $cat_id = $c['categoria_gasto_id'] ?? null;
    if (!$cat_id) {
        $svCat = $pdo->prepare("SELECT id FROM categorias_gastos
            WHERE cliente_id = ? AND activa = 1 AND LOWER(nombre) LIKE '%nomina%' LIMIT 1");
        $svCat->execute([$cid]);
    ...
    $archivo_nombre  = null;
    $fileKey = isset($_FILES['comprobante']) ? 'comprobante'
        : (isset($_FILES['archivo_adjunto']) ? 'archivo_adjunto' : null);
    ...
    if ($fileKey && isset($_FILES[$fileKey]) && $_FILES[$fileKey]['error'] === UPLOAD_ERR_OK) {
        ...
        if (!$mime || !isset($allowed[$mime]))
            throw new Exception("Tipo no permitido. Solo JPG, PNG, WEBP o PDF.");

        $ext       = $allowed[$mime];
        $subDir    = date('Y') . '/' . date('m') . '/';
        $uploadDir = UPLOAD_DIR_BASE . $subDir;
        ...
        $archivo_adjunto = $subDir . $filename;
    ...
    // Insertar gasto principal de nómina
    $stmtIns = $pdo->prepare("
        INSERT INTO gastos
    ...
    // Marcar cuotas como pagadas y actualizar saldos de préstamos
    foreach ($cuotas_auto as $cuota) {
        $pdo->prepare("
            UPDATE colaborador_prestamo_cuotas
            SET estado = 'pagado', fecha_pago = ?, metodo_pago = 'descuento_nomina',
                notas = CONCAT(IFNULL(notas,''), ' | Descontado en nómina gasto #$gasto_id')
            WHERE id = ?
        ")->execute([$fecha, $cuota['cuota_id']]);

        $pdo->prepare("
            UPDATE colaborador_prestamos
            SET saldo_pendiente = GREATEST(0, saldo_pendiente - ?)
            WHERE id = ?
        ")->execute([$cuota['cuota_monto'], $cuota['prestamo_id']]);

        // Cerrar préstamo si saldo = 0
        $pdo->prepare("
            UPDATE colaborador_prestamos
            SET estado = CASE WHEN saldo_pendiente <= 0 THEN 'pagado' ELSE estado END
            WHERE id = ?
        ")->execute([$cuota['prestamo_id']]);
    }

    // Insertar gastos extras por bonos y viáticos
    $stmtExtra = $pdo->prepare("
        INSERT INTO gastos
            (cliente_id, categoria_id, descripcion, monto, fecha,
             ...
             tipo, metodo_pago, proveedor, notas, archivo_adjunto, archivo_nombre, estado, usuario_id)
        VALUES
            (?,?,?,?,?, 'unico',NULL,NULL, NULL,NULL,NULL, 'fijo',?,NULL, ?,NULL,NULL,'pagado',?)
    ");
    ...
    // Calcular patronal para el desglose del response
