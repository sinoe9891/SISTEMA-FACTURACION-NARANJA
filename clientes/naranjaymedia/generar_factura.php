    ...
    <form id="formFactura" action="guardar_factura" method="POST">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body p-4">
                <select name="cai_rango_id" id="cai_rango_id" class="form-select" required>
                    ...
                    ?>
                        <option value="<?= $cai['id'] ?>">
                            <?= htmlspecialchars($cai['cai']) ?> | Rango:
                            <?= $cai['rango_inicio'] ?>–<?= $cai['rango_fin'] ?> | <?= $restantes ?> restantes | Válido
                            hasta: <?= $cai['fecha_limite'] ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center py-3">
                <h6 class="mb-0 fw-bold"><i class="fa-solid fa-calculator me-2 text-primary"></i>Detalle de Servicios
                </h6>
                    ...
                    <div class="producto-item border rounded-3 p-3 mb-3 bg-light">
                        <div class="row g-2 align-items-end">
                            ...
                            <div class="col-12 mt-1">
                                <textarea name="productos[0][detalles]" class="form-control form-control-sm" rows="1"
                                    placeholder="Descripción / mes de servicio (opcional)"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
            ...
            .then(prods => {
                document.querySelectorAll('.producto-item select[name$="[id]"]').forEach(sel => {
                    ...
                    prods.forEach(prod => {
                        ...
                        o.value = prod.id;
                        o.textContent = prod.nombre + (parseFloat(prod.precio) > 0 ?
                            ` — L${parseFloat(prod.precio).toFixed(2)}` : '');
    ...
    document.getElementById('productos-container').addEventListener('change', function(e) {
        if (e.target.tagName === 'SELECT' && e.target.name.includes('[id]')) {
            ...
            if (precio > 0) e.target.closest('.producto-item').querySelector('input[name$="[precio]"]').value =
                precio.toFixed(2);
            calcularTotales();
        }
    });
    ...
    function calcularTotales() {
        ...
        document.querySelectorAll('.producto-item').forEach(item => {
            ...
            const v = parseInt(item.querySelector('select[name$="[id]"]').selectedOptions[0]?.getAttribute(
                'data-isv')) || 0;
            const t = c * p;
            sub += t;
            if (!exo) {
        ...
        fetch('guardar_factura', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    btn.disabled = false;
                    btn.innerHTML = '<i class="fa-solid fa-floppy-disk me-2"></i>Guardar y Generar Factura';
                }
            }).catch(() => {
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        if (montoGet > 0) {
            const inp = document.querySelector('input[name="productos[0][precio]"]');
            if (inp && !parseFloat(inp.value)) {
                inp.value = parseFloat(montoGet).toFixed(2);
                calcularTotales();
            }
        }
    });
