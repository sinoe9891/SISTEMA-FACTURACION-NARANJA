...
/**
 * productos_por_receptor.php
 *
 * GET ?receptor_id=Y             → productos del receptor (comportamiento original)
 * GET ?receptor_id=Y&todos=1     → TODOS los productos del cliente (rotativo/sin_factura)
 * GET ?receptor_id=0             → devuelve [] igual que antes (retrocompatible)
 * GET ?receptor_id=0&todos=1     → TODOS los productos del cliente
 *
 * Ruta: includes/api/productos_por_receptor.php
 */
...
try {
    ...
    // Sin receptor y sin flag todos → vacío (retrocompatible con versión anterior)
    if ($receptor_id <= 0 && !$todos) {
        echo json_encode([]);
        exit;
    }
    ...
    if ($todos) {
        $stmtP = $pdo->prepare("
            ...
            FROM productos_clientes WHERE cliente_id = ? ORDER BY nombre ASC
        ");
